'''
Common decorators for decorating Unittest test fixtures and test methods.
...
Sphinx ``conf.py`` file::
    ...
    from qe_tools.unittest import decorators
...
In order to utilize this functionality, your Unittest test fixture must
...
environment in which Unittest tests are currently being executed.
...
def _add_tags(func, updated_tags=tuple(), coverage_tags=tuple()):
    '''
...
def _get_decorator_for_skipping_test(reason, details, tag_name, environment_affected=''):
    ...
    Args:
        ...
        tag_name (str): The name of the tag that should be applied with the decorator.
    ...
    def decorator(test_case_or_class):
        ...
        test_case_or_class = _add_tags(test_case_or_class, updated_tags=tags, coverage_tags=tags)
...
def _status_tag_expansion(status_tag, target_tag):
...
def _ensure_single_status_tag_used(tags_list):
    '''Verify single status tag used.

    Args:
        tags_list (iterable of str): The list of tags.
    ...
    # Enhance the tags to make tag couplets for more flexible test execution
    status_tag = actual_status_tags.pop()
    new_tags = []
    for a_tag in tags_list:
        new_tags.append(_status_tag_expansion(status_tag, a_tag))
    return new_tags


def _clear_special_tags_from(func):
...
def tags(*tags_list):
    ...
    def tag_decorator(func):
        ...
        updated_tags = _ensure_single_status_tag_used(total_tags)
        _clear_special_tags_from(func)

        func = _add_tags(func, updated_tags=updated_tags, coverage_tags=tags_list)
