...
def make_http_request(http_url, url_endpoint="", request_params=None):
    """Make a GET call to the Jenkins job URL to either run or get the results of a specified Jenkins job.

    Args:
        http_url (str): A str containing the URL of the HTTP request
        url_endpoint (Union[str, int]): The URL endpoint to add at the end of the Jenkins API URL. (optional)
...
def interactive_mode():
    """Run the interactive mode of the script by prompting the user for each input and validating it."""
    ...
    if http_code != 200:
        eprint("Cannot get a list of jobs from Jenkins: HTTP {0}:\n{1}".format(http_code, http_body))
        sys.exit(1)
    ...
    while True:
        ...
        if state == 0:
            ...
            message = "Please select a number for the Jenkins jobs to execute (1-{0}): ".format(len(list_of_jobs))
        ...
        # Ask for all of the job parameters
            ...
            for param in job_params:
                ...
                if "Options are:" in param["description"]:
                    ...
                    selection = _get_choice_selection(start=1, end=len(options), default=default_option_index,
                    ...
                    quick_command.extend(("--{0}".format(param["name"]), options[int(selection) - 1]))
                ...
                # Parameters with no options
                    ...
                    while param["name"] == "url" and not (value_input == "" or URL_REGEX.match(value_input)):
                        value_input = input("Invalid URL. Please enter a valid URL [{0}]: "
                                            "".format(default_value))
                    selected_job_params[param["name"]] = value_input or default_value
                    quick_command.extend(("--{0}".format(param["name"]), value_input or default_value))
                print("")
            state = 2
    ...
    while True:
        ...
        if http_code == 404:
            if job_in_queue:
                ...
                eprint("The job does not seem to be in the Jenkins queue, nor is it being run by Jenkins.")
                eprint(http_body)
                sys.exit(1)
...
def _print_job_output_intermittently(job, job_build_number):
    ...
    log_files_regex = re.compile("(Detailed logs: /var/lib/jenkins/workspace/{0})(/logs/[0-9-_.]+)".format(job["name"]))
    ...
    while not job_is_done:
        if http_fail_count == 5:
            eprint("The tool is having trouble connecting to Jenkins. Please ensure that Jenkins is still up.")
            sys.exit(1)

        http_code, http_body = make_http_request(job["url"], "{0}/consoleText".format(job_build_number))
        if http_code != 200:
            http_fail_count += 1
            time.sleep(5)
            continue
        ...
        if log_files_regex.search(http_body[start:]):
            http_body = log_files_regex.sub("Detailed logs: {0}ws\g<2>/cafe.master.log".format(job["url"]), http_body)
...
def _job_has_correct_parameters(http_body, job_params):
    """Verify that the job parameters and their values pertaining to the selected job ID match the selected ones.
    ...
    This is a rare scenario, but needed to be addressed. An example would be as such:
        ...
        - One or more users run the same job at the same time from the Jenkins web UI, some with different parameters

    In the scenario above, this would ensure that the results retrieved from Jenkins match the same parameters
    selected by User 1. This would avoid any confusions that may come up.

    Args:
        http_body (dict): The JSON response from an HTTP GET call
        job_params (dict): A dict keyed by the name of the parameter with the values being the parameter values
...
def _is_account_device(account_device):
    ...
    raise argparse.ArgumentTypeError("'{0}' is not a valid account:device ID pair (e.g. 990036:534583)"
                                     "".format(account_device))
...
def read_config_and_set_jenkins_globals(config_file):
    ...
    The config contains the base Jenkins URL, as well as a token that is used to access all of the jobs
    ...
    try:
        ...
        eprint("Section or Key {0} is missing from '{1}' config file. File should have the following structure:\n{2}"
               "".format(str(e), config_file, ("[jenkins]\n"
                                               "url=<jenkins_url>\n"
                                               "token=<token>\n")))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", type=str, default="config.cfg", help="The name of the config file")
    args = parser.parse_known_args()[0]
    read_config_and_set_jenkins_globals(args.config)

    if len(sys.argv) > 2:
        parser.add_argument("--job", required=True, type=str, help="The name of the Jenkins job to run")
        args = parser.parse_known_args()[0]
        ...
        try:
            ...
            job_params = {arg: args.__dict__[arg] for arg in args.__dict__ if arg != "job" and arg != "config"}
        finally:
