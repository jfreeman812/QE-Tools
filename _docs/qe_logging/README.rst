...
file name in the top level of the ``base_log_dir``.  The second handlers will write to a new file
...
The default ``base_log_dir`` of 'logs' can be overridden if desired to have the logs placed in an
alternate location.
