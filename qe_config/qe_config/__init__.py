...
Application or test configuration is often dependent upon the specific environment being used.
Across many environments, there can be a good deal of common configuration as well.
Cut'n'paste-ing configuration files and trying to maintain the various duplicated
configurations is a challenge, and is not DRY. ``load_cake`` gives you a way to layer
configurations so you can have common configurations in one place,
and use them, with variations, in many places.
For convenience a layering of configuration files is called a config cake, or just a cake.

Additionally, ``load_cake``'s configuration file can declare that environment variables can
be used to override configuration settings. This makes it easy to have a cake
...
``load_cake`` allows your code to specify your layering in an external 'master' config file.
The 'master' config file also lets you explicitly configure if, and how,
...
If you want to change the configuration dynamically after that, use ``ConfigParser``'s methods,
...
master.config::
    ...
    all keys = and values in this section will be in the defaults of the new config parser
...
def _filenames_relative_to(base_file, filename_list):
    ...
    return [os.path.join(base_dir, os.path.expanduser(filename)) for filename in filename_list]


def _must_read(config, filename):
    # Python v2 and v3 don't have a common .read_file() function...
...
def munchify_config(config_parser):
    ...
    Note: This is a one-time conversion, if the ``config_parser`` is subsequently updated,
    the returned ``munch`` will be unaffected.
    '''
    from munch import Munch
    result = Munch()

    try:  # Python 3 has changable name for the defaults section
        default_section = config_parser.default_section
    except AttributeError:
        default_section = 'DEFAULT'

    for section in config_parser.sections() + [default_section]:
        intermediate = Munch()
